...
'''
PART 1:
generating a random secret-key sk from a probability distribution, 
we will use the uniform distribution over R2, which means sk will be a polynomial 
with coefficients being 0 or 1. For the public-key we first sample a polynomial a uniformly 
over Rq and a small error polynomial e from a discrete normal distribution over Rq. 
We then set the public-key to be the tuple pk=([−(a⋅sk+e)]q,a). 
